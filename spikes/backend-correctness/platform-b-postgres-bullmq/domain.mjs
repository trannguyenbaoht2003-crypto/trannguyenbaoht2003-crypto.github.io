...
const TABLES = [
  ...
  'idempotency_records', 'failure_controls', 'dead_letters', 'fixture_blob', 'fixture_records',
];
...
DROP TABLE IF EXISTS source_policies, raw_observations, candidates, candidate_provenance, claims,
  ...
  eligibility_evaluations, candidate_current_eligibility, publications, publication_versions,
  active_publications, candidate_projection, audit_events, outbox_events, consumer_effects,
  idempotency_records, failure_controls, dead_letters, fixture_blob, fixture_records, atomic_failure_sentinel CASCADE;
...
CREATE TABLE fixture_records (record_kind TEXT NOT NULL, record_id TEXT NOT NULL, payload_json TEXT NOT NULL, PRIMARY KEY(record_kind, record_id));
...
export function createPostgresDomain({ pool, enqueue }) {
  ...
  async function snapshot() {
    ...
    const fixtureRows = await pool.query('SELECT record_kind, COUNT(*) AS total FROM fixture_records GROUP BY record_kind ORDER BY record_kind');
    const fixtureCounts = Object.fromEntries(fixtureRows.rows.map((row) => [row.record_kind, Number(row.total)]));
    return { outboxEvents: counts.outbox_events, consumerEffects: counts.consumer_effects, deliveredOutboxEvents: Number(delivered.rows[0]?.total ?? 0), counts, fixtureCounts, activePublications: active.rows, currentEligibility: eligibility.rows, currentModeration: moderation.rows, candidateProjection: projections.rows };
  }
  ...
  async function loadFixture(body) {
    ...
    try {
      await client.query('BEGIN');
      await client.query('DELETE FROM fixture_records');
      for (const [kind, records] of Object.entries(body.fixture ?? {})) {
        if (!Array.isArray(records)) continue;
        for (let index = 0; index < records.length; index += 200) {
          const batch = records.slice(index, index + 200);
          const values = [];
          const placeholders = batch.map((record, offset) => {
            const base = values.length;
            const recordId = record.id ?? record.publicationId ?? `${kind}-${index + offset}`;
            values.push(kind, String(recordId), JSON.stringify(record));
            return `($${base + 1}, $${base + 2}, $${base + 3})`;
          });
          if (placeholders.length) {
            await client.query(`INSERT INTO fixture_records (record_kind, record_id, payload_json) VALUES ${placeholders.join(', ')}`, values);
          }
        }
      }
      await client.query("INSERT INTO fixture_blob (fixture_id, fixture_json, counts_json, checksum, created_at) VALUES ('common', $1, $2, $3, $4) ON CONFLICT(fixture_id) DO UPDATE SET fixture_json = EXCLUDED.fixture_json, counts_json = EXCLUDED.counts_json, checksum = EXCLUDED.checksum, created_at = EXCLUDED.created_at", [JSON.stringify(body.fixture), JSON.stringify(body.counts), body.checksum, createdAt]);
      await client.query('COMMIT');
    } catch (error) {
      await client.query('ROLLBACK');
      throw error;
    ...
    const materialized = await pool.query('SELECT COUNT(*) AS total FROM fixture_records');
    return { ok: true, counts: body.counts, checksum: body.checksum, materializedRecords: Number(materialized.rows[0]?.total ?? 0) };
  }
