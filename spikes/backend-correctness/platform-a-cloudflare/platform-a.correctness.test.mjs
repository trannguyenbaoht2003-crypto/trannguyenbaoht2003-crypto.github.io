...
import { createEvidenceRecorder } from '../common/evidence-recorder.mjs';
import { createCloudflareAdapter } from './adapter.mjs';

let adapter;
const recorder = createEvidenceRecorder({ platform: 'cloudflare-worker-d1-queues' });

before(async () => {
  adapter = await createCloudflareAdapter({ mode: 'local-test' });
});
...
afterEach(async (context) => {
  await recorder.complete({
    ...
    snapshot: await adapter.snapshotState(),
    evidence: await adapter.collectEvidence(),
  });
});
...
function meta(prefix, type) {
  return {
    type,
    auditId: `audit-${prefix}-${type}`,
    eventId: `event-${prefix}-${type}`,
  };
}
...
async function createCandidate(prefix, options = {}) {
  ...
  await adapter.executeCommand({
    ...meta(prefix, 'register_candidate'),
    candidateId: `candidate-${prefix}`,
    fingerprint: options.fingerprint ?? `fingerprint-${prefix}`,
    patchId,
    catalogRevisionId: options.catalogRevisionId ?? `catalog-${patchId}`,
    origin: options.origin ?? 'editorial',
    catalogValid: options.catalogValid ?? true,
    provenanceId: `provenance-${prefix}-1`,
    sourceRef: options.sourceRef ?? `source-${prefix}`,
    claims,
  });
  ...
  for (let index = 0; index < claimStates.length; index += 1) {
    ...
    await adapter.executeCommand({
      ...
      decisionId: `evidence-${prefix}-${index + 1}`,
      claimId: claims[index].claimId,
      patchId: options.evidencePatchId ?? patchId,
      state,
  ...
  for (let index = 0; index < (options.reviews ?? 0); index += 1) {
    await adapter.executeCommand({
      ...
      reviewId: `review-${prefix}-${index + 1}`,
      candidateId: `candidate-${prefix}`,
  ...
  if (options.evaluate !== false) {
    evaluation = await adapter.executeCommand({
      ...
      candidateId: `candidate-${prefix}`,
      reviewQuorum: options.reviewQuorum ?? 1,
    });
  }
...
async function scenario(id) {
  ...
  await recorder.begin(found, await adapter.snapshotState());
  return found;
}
...
await scenario('S1');
  for (const point of ['F01', 'F02']) {
    ...
    await assert.rejects(createCandidate(`s1-${point}`, { evaluate: false }), /PLATFORM_A_REQUEST_FAILED/);
...
await scenario('S2');
  ...
  const command = {
    ...meta('s2-observation', 'ingest_observation'),
    observationId: 'observation-s2',
    idempotencyKey: 'idempotency-s2',
    payloadHash: 'payload-hash-s2',
    sourceId: 'source-s2',
    reference: { url: 'https://example.invalid/s2' },
    blobText: 'allowed raw body',
  };
...
test('S3 idempotency payload conflict is rejected without state mutation', async () => {
await scenario('S3');
  ...
  const command = {
    ...meta('s3-observation', 'ingest_observation'),
    observationId: 'observation-s3', idempotencyKey: 'idempotency-s3', payloadHash: 'hash-a',
    sourceId: 'source-s3', reference: { value: 'a' }, blobText: 'must-not-store',
  };
...
test('S4 outbox remains consistent across F03/F04/F05 and F11 retry exhaustion', async () => {
await scenario('S4');
  ...
  assert.equal(state.outboxEvents, 1);
...
test('S5 concurrent review has no lost writes across 20 iterations', async () => {
await scenario('S5');
...
test('S6 evidence support remains independent from blocked moderation', async () => {
await scenario('S6');
...
test('S7 AI candidate cannot publish without confirmed human review', async () => {
await scenario('S7');
  const seeded = await createCandidate('s7', { origin: 'ai_generated', moderation: 'clear', reviews: 0 });
...
test('S8 publisher permission is mandatory and cannot alter eligibility', async () => {
await scenario('S8');
  const seeded = await createCandidate('s8', { moderation: 'clear' });
  assert.equal(seeded.evaluation.result, 'eligible');
  await assert.rejects(publishCandidate('s8', seeded, { actorPermissions: [] }), /PUBLISHER_PERMISSION_REQUIRED/);
  const state = await adapter.snapshotState();
  assert.equal(state.currentEligibility[0].result, 'eligible');
  assert.equal(state.counts.publication_versions, 0);
});

test('S9 stale moderation input blocks publication', async () => {
await scenario('S9');
...
test('S10 immutable versions rollback atomically and idempotently', async () => {
await scenario('S10');
  const seeded = await createCandidate('s10', { moderation: 'clear' });
  await adapter.injectFailure('F08');
  await assert.rejects(publishCandidate('s10', seeded), /PLATFORM_A_REQUEST_FAILED/);
  assert.equal((await adapter.snapshotState()).counts.publication_versions, 0);
  ...
  const rollback = {
    ...meta('s10', 'rollback'), publicationId: 'publication-s10', targetVersionId: 'version-s10-1',
    actorPermissions: ['publisher'], idempotencyKey: 'rollback-s10', payloadHash: 'rollback-hash-s10',
  };
  await adapter.executeCommand(rollback);
  const replay = await adapter.executeCommand(rollback);
...
test('S11 reference-only policy prevents blob storage', async () => {
await scenario('S11');
  ...
  const result = await adapter.executeCommand({
    ...meta('s11-observation', 'ingest_observation'), observationId: 'observation-s11', idempotencyKey: 'idempotency-s11', payloadHash: 'hash-s11',
    sourceId: 'source-s11', reference: { url: 'https://example.invalid/reference' }, blobText: 'forbidden blob',
  });
...
await scenario('S12');
  const seeded = await createCandidate('s12', { catalogValid: false, moderation: 'clear' });
  assert.equal(seeded.evaluation.result, 'ineligible');
});

test('S13 backup and restore preserve full canonical checksum', async () => {
await scenario('S13');
  ...
  assert.equal(fixture.materializedRecords, Object.values(fixture.counts).reduce((sum, value) => sum + value, 0));
  assert.deepEqual((await adapter.snapshotState()).fixtureCounts, fixture.counts);
  const backup = await adapter.backupState();
  const before = await adapter.computeChecksums();
...
test('S14 public read remains available before pipeline consumers run', async () => {
await scenario('S14');
  const seeded = await createCandidate('s14', { moderation: 'clear' });
  await publishCandidate('s14', seeded, { content: { title: 'Published S14' } });
  const publicRows = await adapter.readPublishedContent();
  assert.equal(publicRows.length, 1);
  assert.equal(publicRows[0].content.title, 'Published S14');
...
test('S16 first moderation evaluation may explicitly produce all three states', async () => {
await scenario('S16');
  assert.equal((await adapter.snapshotState()).counts.moderation_decisions, 0);
  for (const state of ['clear', 'flagged', 'blocked']) await createCandidate(`s16-${state}`, { moderation: state, evaluate: false });
  const states = (await adapter.snapshotState()).currentModeration.map((entry) => entry.state).sort();
  assert.deepEqual(states, ['blocked', 'clear', 'flagged']);
});

test('S17 moderation decisions pin immutable input snapshots', async () => {
await scenario('S17');
...
test('S18 superseded clear decision cannot race into publication', async () => {
await scenario('S18');
...
test('S19 claim-level aggregation refuses candidate-level shortcuts', async () => {
await scenario('S19');
...
test('S20 cross-patch evidence must be re-evaluated', async () => {
await scenario('S20');
  ...
  const reevaluated = await adapter.executeCommand({ ...meta('s20-second', 'evaluate_eligibility'), evaluationId: 'eligibility-s20-2', candidateId: seeded.candidateId });
  assert.equal(reevaluated.result, 'eligible');
});

test('S21 origin-independent fingerprint deduplicates candidate while retaining provenance', async () => {
await scenario('S21');
  ...
  const duplicate = await adapter.executeCommand({
    ...meta('s21-ai', 'register_candidate'), candidateId: 'candidate-s21-ai', fingerprint: 'shared-fingerprint-s21', patchId: 'patch-1', catalogRevisionId: 'catalog-patch-1',
    origin: 'ai_generated', catalogValid: true, provenanceId: 'provenance-s21-ai', sourceRef: 'ai', claims: [],
  });
...
test('S22 monitoring is a single idempotent projection of PublicationPublished', async () => {
await scenario('S22');
  const seeded = await createCandidate('s22', { moderation: 'clear' });
  await publishCandidate('s22', seeded);
  assert.equal((await adapter.readPublishedContent()).length, 1);
  assert.equal((await adapter.snapshotState()).candidateProjection[0].monitoring, 0);
...
test('S23 AI quorum requires two distinct confirmed reviewers when policy says two', async () => {
await scenario('S23');
  ...
  const first = await adapter.executeCommand({ ...meta('s23-first', 'evaluate_eligibility'), evaluationId: 'eligibility-s23-1', candidateId: seeded.candidateId, reviewQuorum: 2 });
  ...
  const second = await adapter.executeCommand({ ...meta('s23-second', 'evaluate_eligibility'), evaluationId: 'eligibility-s23-2', candidateId: seeded.candidateId, reviewQuorum: 2 });
  assert.equal(second.result, 'eligible');
});
...
await scenario('S24');
  ...
  const active = (await adapter.snapshotState()).activePublications;
