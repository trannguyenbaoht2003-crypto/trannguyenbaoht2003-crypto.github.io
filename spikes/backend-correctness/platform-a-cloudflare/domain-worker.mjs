...
DROP TABLE IF EXISTS fixture_records;
...
async function snapshot(env) {
  ...
  const fixtureRows = await env.DB.prepare('SELECT record_kind, COUNT(*) AS total FROM fixture_records GROUP BY record_kind ORDER BY record_kind').all();
  const fixtureCounts = Object.fromEntries((fixtureRows.results ?? []).map((row) => [row.record_kind, Number(row.total)]));
  return { outboxEvents: counts.outbox_events, consumerEffects: counts.consumer_effects, deliveredOutboxEvents: Number(delivered?.total ?? 0), counts, fixtureCounts, activePublications: active.results ?? [], currentEligibility: eligibility.results ?? [], currentModeration: moderation.results ?? [], candidateProjection: projections.results ?? [] };
}
...
async function loadFixture(request, env) {
  ...
  await env.DB.prepare('DELETE FROM fixture_records').run();
  for (const [kind, records] of Object.entries(body.fixture ?? {})) {
    if (!Array.isArray(records)) continue;
    for (let index = 0; index < records.length; index += 50) {
      const statements = records.slice(index, index + 50).map((record, offset) => {
        const recordId = record.id ?? record.publicationId ?? `${kind}-${index + offset}`;
        return env.DB.prepare('INSERT INTO fixture_records (record_kind, record_id, payload_json) VALUES (?, ?, ?)')
          .bind(kind, String(recordId), JSON.stringify(record));
      });
      if (statements.length) await env.DB.batch(statements);
    }
  }
  ...
  const materialized = await env.DB.prepare('SELECT COUNT(*) AS total FROM fixture_records').first();
  return { ok: true, counts: body.counts, checksum: body.checksum, materializedRecords: Number(materialized?.total ?? 0) };
}
