...
const outputDir = path.resolve('spikes/backend-correctness/platform-a-cloudflare/evidence');
...
try {
  ...
  await Promise.all([
    writeJson('run-manifest.json', {
      ...
      commonHarnessSha: PLATFORM_A_METADATA.commonHarnessSha,
  ...
  if (!restorePassed || mutationAuditCount !== 8 || duplicateEvent?.transition_count !== 1) {
    throw new Error('PLATFORM_A_EVIDENCE_INVARIANT_FAILED');
  }
} finally {
  await adapter.close();
}
