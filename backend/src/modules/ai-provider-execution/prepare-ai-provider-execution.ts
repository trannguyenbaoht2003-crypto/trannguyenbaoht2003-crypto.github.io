...
function budgetResult(
  ...
  const date = row.budget_date instanceof Date
    ? row.budget_date.toISOString().slice(0, 10)
...
function assertExistingIdentity(
  ...
  if (
    row.ai_discovery_run_id !== command.aiDiscoveryRunId
    || row.run_key !== command.runKey
    || row.provider_key !== command.providerKey
    || row.model_key !== command.modelKey
    || row.model_revision !== command.modelRevision
    || row.prompt_template_key !== command.promptTemplateKey
    || row.prompt_template_version !== command.promptTemplateVersion
    || row.input_hash !== command.inputHash
  ) {
...
async function loadExisting(
  pool: Pool,
  command: PrepareAiProviderExecutionCommand,
): Promise<PrepareAiProviderExecutionResult | null> {
  const result = await pool.query<ExistingRow>(
    `select e.ai_provider_execution_id,e.ai_discovery_run_id,e.run_key,
            e.provider_key,e.model_key,e.model_revision,e.prompt_template_key,
            e.prompt_template_version,e.input_hash,e.status,e.current_attempt_ordinal,
      ...
      where e.ai_discovery_run_id=$1 or e.run_key=$2`,
    [command.aiDiscoveryRunId, command.runKey],
  );
  if (result.rowCount === 0) return null;
  if (result.rowCount !== 1) throw new Error('AI_PROVIDER_EXECUTION_IDENTITY_CONFLICT');
  const row = result.rows[0]!;
  assertExistingIdentity(command, row);
  return {
    kind: 'EXISTING',
    executionId: row.ai_provider_execution_id,
    status: row.status,
    currentAttemptOrdinal: row.current_attempt_ordinal,
    reservation: budgetResult(command, row),
  };
}
...
export async function prepareAiProviderExecution(
  ...
  if (historical.rowCount !== 0) {
    throw new Error('AI_PROVIDER_EXECUTION_HISTORICAL_AUTHORIZATION_CONSUMED');
  }
    ...
    return await withTransaction(pool, async (client) => {
      ...
      await client.query(
        ...
        [
          executionId,
          command.aiDiscoveryRunId,
          reservation.aiOperationsRunBudgetReservationId,
          command.runKey,
      ...
      await client.query(
        ...
        [attemptId, executionId, clientRequestId],
      );
      return {
        kind: 'PREPARED',
        executionId,
