...
import {
  recordAiDiscoveryRunInTransaction,
} from '../ai-discovery/record-ai-discovery-run.js';
import type {
...
export interface FinalizeAiProviderExecutionCommand {
  ...
  completedRun?: RecordAiDiscoveryRunCommand;
  failedRun?: RecordAiDiscoveryRunCommand;
}
...
export type FinalizeAiProviderExecutionResult =
  ...
  | { kind: 'RETRY_PREPARED'; attemptId: string; ordinal: AiProviderAttemptOrdinal; clientRequestId: string }
  | { kind: 'UNCERTAIN' };
...
export async function finalizeAiProviderExecution(
  ...
  return withTransaction(pool, async (client) => {
    const locked = await client.query<{
      ...
      `select status,current_attempt_ordinal,lease_token,lease_expires_at
    ...
    if (!execution || execution.status !== 'IN_FLIGHT' || execution.current_attempt_ordinal !== command.ordinal) {
    ...
    if (command.disposition.kind === 'COMPLETED') {
      if (!command.completedRun) throw new Error('AI_PROVIDER_EXECUTION_FINALIZATION_INPUT_INVALID');
      ...
      await client.query(
        `update ai_provider_execution_attempts
          ...
          where ai_provider_execution_attempt_id=$1 and ai_provider_execution_id=$2 and status='IN_FLIGHT'`,
        [command.attemptId,command.executionId,command.disposition.result.providerRequestId,
         command.disposition.result.providerResponseId,command.completedRun.outputHash],
      );
      await client.query(
    ...
    if (command.disposition.kind === 'SAFE_TERMINAL') {
      if (!command.failedRun) throw new Error('AI_PROVIDER_EXECUTION_FINALIZATION_INPUT_INVALID');
      const run = await recordAiDiscoveryRunInTransaction(client,command.failedRun);
      await client.query(
        `update ai_provider_execution_attempts
            set status='FAILED',failure_code=$3,provider_request_id=$4,completed_at=clock_timestamp()
          where ai_provider_execution_attempt_id=$1 and ai_provider_execution_id=$2 and status='IN_FLIGHT'`,
    ...
    if (command.disposition.kind === 'SAFE_RETRYABLE') {
      ...
      await client.query(
        `insert into ai_provider_execution_attempts
         ...
         values ($1,$2,$3,$4,'PREPARED')`,[nextAttemptId,command.executionId,nextOrdinal,clientRequestId],
      );
      const leaseStillValid=execution.lease_token!==null
      ...
      await client.query(
        `update ai_provider_executions
            set status='PREPARED',current_attempt_ordinal=$2,
                ...
                leased_at=case when $3 then leased_at else null end,
                lease_expires_at=case when $3 then lease_expires_at else null end,
                updated_at=clock_timestamp()
          where ai_provider_execution_id=$1`,[command.executionId,nextOrdinal,leaseStillValid],
      );
      return {kind:'RETRY_PREPARED',attemptId:nextAttemptId,ordinal:nextOrdinal,clientRequestId};
    }

    await client.query(
      `update ai_provider_execution_attempts
          set status='UNCERTAIN',failure_code=$3,provider_request_id=$4,completed_at=clock_timestamp()
        where ai_provider_execution_attempt_id=$1 and ai_provider_execution_id=$2 and status='IN_FLIGHT'`,
      [command.attemptId,command.executionId,command.disposition.failureCode,providerRequestId],
    );
    await client.query(
