...
function failInput(): never {
  throw new Error('AI_OPERATIONS_BUDGET_INPUT_INVALID');
}
    ...
    return await withTransaction(pool, async (client) => {
      ...
      await client.query(
        `select pg_advisory_xact_lock(
           hashtextextended('ai_operations_provider_budget:v1', 0)
         )`,
      );
      ...
      if (policy.game_mode_external_id !== command.gameModeExternalId) {
        throw new Error('AI_OPERATIONS_GAME_MODE_NOT_ALLOWED');
      }
      ...
      const state = await client.query<BudgetStateRow>(
        `select (timezone('UTC', clock_timestamp()))::date::text as budget_date,
                ...
                extract(epoch from (
                  clock_timestamp() - max(all_runs.reserved_at)
                ))::text as seconds_since_last
      ...
      const inserted = await client.query<{ budget_date: string | Date }>(
        ...
        [
          ...
          command.aiDiscoveryRunId,
          command.runKey,
          policy.ai_operations_policy_revision_id,
          policy.max_proposals_per_run,
          command.actorId,
