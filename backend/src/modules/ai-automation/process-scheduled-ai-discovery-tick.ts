...
} from './build-scheduled-ai-discovery-input.js';
...
export type ScheduledAiDiscoveryTickOutcome =
  | 'DUPLICATE_NOOP'
  | 'NO_NEW_INPUT'
  | 'CADENCE_NOT_ELAPSED'
  | 'POLICY_DISABLED'
...
export interface ProcessScheduledAiDiscoveryTickCommand {
  actorId: string;
...
export interface ProcessScheduledAiDiscoveryTickResult {
  outcome: ScheduledAiDiscoveryTickOutcome;
...
type ExecuteRun = (
  pool: Pool,
  command: Parameters<typeof executePolicyGovernedAiDiscoveryRun>[1],
  dependencies?: ExecutePolicyGovernedAiDiscoveryRunDependencies,
) => Promise<ExecutePolicyGovernedAiDiscoveryRunResult>;

export interface ProcessScheduledAiDiscoveryTickDependencies {
  buildInput?: BuildInput;
  executeRun?: ExecuteRun;
  newId?: () => string;
}

interface TickClaimRow {
  scheduled_ai_discovery_tick_id: string;
}
...
function knownBudgetOutcome(error: unknown): Exclude<
  ScheduledAiDiscoveryTickOutcome,
  'DUPLICATE_NOOP' | 'NO_NEW_INPUT' | 'COMPLETED' | 'PROVIDER_FAILED' | 'AMBIGUOUS_FAILURE'
> | null {
...
async function claimTick(pool: Pool, id: string): Promise<string | null> {
  const result = await pool.query<TickClaimRow>(
    `insert into scheduled_ai_discovery_ticks
      (scheduled_ai_discovery_tick_id, scheduler_key, utc_hour, status)
     values ($1, $2, date_trunc('hour', clock_timestamp()), 'PROCESSING')
     on conflict (scheduler_key, utc_hour) do nothing
     returning scheduled_ai_discovery_tick_id`,
    [id, SCHEDULER_KEY],
  );
  return result.rows[0]?.scheduled_ai_discovery_tick_id ?? null;
}

async function enrichTick(
  pool: Pool,
  tickId: string,
  input: BuiltScheduledAiDiscoveryInput,
): Promise<void> {
  await pool.query(
    ...
    [tickId, input.scheduledContentHash, input.aiDiscoveryRunId],
  );
}

async function contentWasBudgetConsumed(
  pool: Pool,
  tickId: string,
...
async function reservationForRun(
  ...
  const result = await pool.query<ReservationRow>(
    `select ai_operations_run_budget_reservation_id,
...
async function finalizeTick(
  pool: Pool,
  tickId: string,
  outcome: Exclude<ScheduledAiDiscoveryTickOutcome, 'DUPLICATE_NOOP'>,
  links: {
  ...
  await pool.query(
    `update scheduled_ai_discovery_ticks
        set status = $2,
            ai_operations_policy_revision_id = coalesce($3, ai_operations_policy_revision_id),
            ai_operations_run_budget_reservation_id = coalesce($4, ai_operations_run_budget_reservation_id),
    ...
    [
      tickId,
      outcome,
      links.policyRevisionId ?? null,
      links.budgetReservationId ?? null,
    ],
  );
}

export async function processScheduledAiDiscoveryTick(
  pool: Pool,
  command: ProcessScheduledAiDiscoveryTickCommand,
  dependencies: ProcessScheduledAiDiscoveryTickDependencies = {},
): Promise<ProcessScheduledAiDiscoveryTickResult> {
  const tickId = await claimTick(pool, (dependencies.newId ?? randomUUID)());
  if (!tickId) {
    return { outcome: 'DUPLICATE_NOOP', scheduledAiDiscoveryTickId: null, aiDiscoveryRunId: null };
  }
  ...
  try {
    ...
    if (!built) {
      await finalizeTick(pool, tickId, 'NO_NEW_INPUT');
      return { outcome: 'NO_NEW_INPUT', scheduledAiDiscoveryTickId: tickId, aiDiscoveryRunId: null };
    }

    await enrichTick(pool, tickId, built);
    if (await contentWasBudgetConsumed(pool, tickId, built.scheduledContentHash)) {
      await finalizeTick(pool, tickId, 'NO_NEW_INPUT');
      return {
        outcome: 'NO_NEW_INPUT',
        scheduledAiDiscoveryTickId: tickId,
      ...
      },
      {
        reserveBudget: (budgetPool, budgetCommand) => (
          reserveAiOperationsRunBudgetWithFloor(
            budgetPool,
            budgetCommand,
            { minimumIntervalFloorSeconds: SCHEDULED_INTERVAL_SECONDS },
          )
        ),
      },
    );
    ...
    await finalizeTick(pool, tickId, outcome, {
    ...
    if (budgetOutcome) {
      await finalizeTick(pool, tickId, budgetOutcome);
      return {
        outcome: budgetOutcome,
        scheduledAiDiscoveryTickId: tickId,
        aiDiscoveryRunId: built?.aiDiscoveryRunId ?? null,
      };
    }

    const reservation = built
      ? await reservationForRun(pool, built.aiDiscoveryRunId)
      : null;
    await finalizeTick(pool, tickId, 'AMBIGUOUS_FAILURE', reservation ? {
      policyRevisionId: reservation.ai_operations_policy_revision_id,
      budgetReservationId: reservation.ai_operations_run_budget_reservation_id,
    } : {});
    return {
