...
async function contextCurrent(pool: Pool, run: AutonomousRun): Promise<boolean> {
    try {
        ...
        }>(`select moderation_decision_id from current_candidate_moderation_decisions where candidate_revision_id=$1 and moderation_policy_revision_id=$2`, [run.candidate_revision_id, run.moderation_policy_revision_id])).rows[0]?.moderation_decision_id ?? null;
        return moderation === run.expected_moderation_decision_id || moderation === runUuid(run, 'moderation');
    }
    catch (error) {
...
async function finalizeResponse(pool: Pool, run: AutonomousRun): Promise<void> {
    ...
    try {
        ...
        try {
            ...
            const published = await pool.query(`select 1 from idempotency_records where scope='publication_publish' and idempotency_key=$1 and state='completed'`, [command.idempotencyKey]);
...
async function executeReserved(pool: Pool, run: AutonomousRun, provider: AiReviewProvider, clock: ReturnType<typeof autonomousClock>) {
    ...
    const owned = await pool.query<AutonomousRun>(`update autonomous_ai_review_runs set state='in_flight',in_flight_at=$2 where run_id=$1 and state='reserved' returning *`, [run.run_id, clock.now]);
    if (!owned.rowCount)
        return;
...
export async function processAutonomousReviewTick(pool: Pool, options: {
    ...
    for (const run of recovery) {
        if (run.state === 'responded')
            ...
            await executeReserved(pool, run, options.provider, clock);
    }
    ...
    for (const candidate of await scanCandidates(pool)) {
        ...
        if (duplicate.rowCount)
            continue;
        const run = await reserveRun(pool, prepared, options.model, clock);
        if (!run)
            return { outcome: 'BUDGET_OR_TICK_RESERVED' };
        await executeReserved(pool, run, options.provider, clock);
