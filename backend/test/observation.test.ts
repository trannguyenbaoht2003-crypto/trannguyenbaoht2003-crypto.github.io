...
for (const adapterVersion of ['community-collector-bridge-v1', 'community-collector-bridge-v2']) {
  test(`${adapterVersion} content identity replays across a changed collection timestamp`, async (t) => {
    const pool = await seedPolicy('blob_allowed');
    t.after(() => pool.end());
    ...
    assert.deepEqual((await pool.query('select collected_at from raw_observations')).rows,
      [{ collected_at: new Date('2026-07-23T01:00:00Z') }]);
  });

  test(`${adapterVersion} legacy receipt replays from its persisted collection timestamp only`, async (t) => {
    const pool = await seedPolicy('blob_allowed');
    t.after(() => pool.end());
    const { rawBlob: _rawBlob, ...communityCommand } = command();
    void _rawBlob;
    ...
    await assert.rejects(
      ingestObservation(pool, {
        ...communityCommand,
        adapterVersion: adapterVersion === 'community-collector-bridge-v1'
          ? 'community-collector-bridge-v2' : 'community-collector-bridge-v1',
        idempotencyKey: payload.idempotencyKey,
        collectedAt: new Date('2026-07-24T01:00:00Z'),
      }),
      /IDEMPOTENCY_PAYLOAD_CONFLICT/,
...
test('unknown adapters retain collection timestamp as part of their idempotency payload', async (t) => {
  const pool = await seedPolicy('blob_allowed');
  t.after(() => pool.end());
  const input = { ...command(), adapterVersion: 'community-collector-bridge-v3' };
  await ingestObservation(pool, input);
  await assert.rejects(
    ingestObservation(pool, { ...input, collectedAt: new Date('2026-07-24T01:00:00Z') }),
    /IDEMPOTENCY_PAYLOAD_CONFLICT/,
  );
  assert.equal(await tableCount(pool, 'raw_observations'), 1);
  assert.equal(await tableCount(pool, 'audit_events'), 1);
